...
def test_billing_ticket():
    ...
    n8nTutorial: [
      ...
      {
        nodeLabel: "HTTP Request — Contexte CRM",
        nodeType: "HTTP Request",
        nodeIcon: "🌐",
        description: "Ce nœud récupère les informations du client depuis votre CRM pour enrichir le contexte avant la classification. L'historique client permet une meilleure priorisation.",
        configuration: `1. Ajoutez un nœud "HTTP Request"\n2. Méthode : GET\n3. URL : https://votre-crm.com/api/contacts/{{ $json.email }}\n4. Authentication : Predefined Credential Type\n5. Créez un credential de type "Header Auth" avec votre clé API CRM\n6. Options > Response Format : JSON\n7. Options > Timeout : 10000 (10 secondes)\n8. Connectez la sortie du Webhook vers ce nœud`,
        customization: `• HubSpot : GET https://api.hubapi.com/contacts/v1/contact/email/{{ $json.email }}/profile — utilisez le credential "HubSpot API" natif de n8n\n• Pipedrive : GET https://api.pipedrive.com/v1/persons/search?term={{ $json.email }} — utilisez le credential Pipedrive natif\n• Salesforce : utilisez le nœud "Salesforce" natif au lieu de HTTP Request\n• Sans CRM : supprimez ce nœud et passez directement au nœud suivant — le triage fonctionnera, mais sans contexte client`,
        errorHandling: `• Erreur 401/403 : votre clé API CRM est expirée ou n'a pas les bonnes permissions\n• Erreur 404 (contact inconnu) : ajoutez un nœud "IF" après celui-ci pour gérer le cas où le client n'existe pas dans le CRM → passez un contexte vide\n• Timeout : augmentez la valeur ou vérifiez que votre CRM est accessible depuis le serveur n8n`,
      },
      {
      ...
      {
        nodeLabel: "HTTP Request — Appel LLM (OpenAI)",
        nodeType: "HTTP Request",
        nodeIcon: "🤖",
        description: "Ce nœud envoie le prompt au LLM (OpenAI GPT-4) et récupère la classification structurée en JSON. C'est le cœur du workflow.",
        configuration: `1. Ajoutez un nœud "HTTP Request"\n2. Méthode : POST\n3. URL : https://api.openai.com/v1/chat/completions\n4. Authentication : Predefined Credential Type > "OpenAI API"\n5. Créez un credential OpenAI avec votre clé API\n6. Send Headers : Content-Type = application/json\n7. Send Body : JSON\n8. Body :\n{\n  "model": "gpt-4o-mini",\n  "temperature": 0,\n  "response_format": { "type": "json_object" },\n  "messages": [\n    { "role": "user", "content": "{{ $json.prompt }}" }\n  ]\n}\n9. Options > Timeout : 30000 (30 secondes)`,
        customization: `• Pour utiliser Claude au lieu de GPT : changez l'URL en https://api.anthropic.com/v1/messages et adaptez le body au format Anthropic (voir doc Anthropic)\n• Pour utiliser Mistral (option européenne 🇪🇺) : URL https://api.mistral.ai/v1/chat/completions, model "mistral-large-latest"\n• Pour un LLM gratuit/local avec Ollama : URL http://localhost:11434/v1/chat/completions, model "llama3"\n• gpt-4o-mini est le meilleur rapport qualité/prix pour du triage — passez à gpt-4o uniquement si la précision est insuffisante`,
        errorHandling: `• Erreur 401 : clé API invalide ou expirée — vérifiez dans votre dashboard OpenAI\n• Erreur 429 (rate limit) : vous envoyez trop de requêtes — ajoutez un nœud "Wait" de 1s avant cet appel, ou augmentez vos limites OpenAI\n• Erreur 500 : problème côté OpenAI — ajoutez un nœud "IF" de retry : si erreur, attendez 5s et relancez\n• Réponse non-JSON : vérifiez que "response_format" est bien configuré ; si le LLM refuse, reformulez le prompt avec "Réponds UNIQUEMENT en JSON"`,
      },
      {
      ...
      {
        nodeLabel: "HTTP Request — Mise à jour Zendesk",
        nodeType: "HTTP Request",
        nodeIcon: "🎫",
        description: "Ce nœud met à jour le ticket dans votre système de support avec la classification IA : catégorie, urgence, équipe assignée, et suggestion de réponse.",
        configuration: `1. Ajoutez un nœud "HTTP Request"\n2. Méthode : PUT\n3. URL : https://votre-instance.zendesk.com/api/v2/tickets/{{ $json.ticketId }}\n4. Authentication : Predefined Credential Type > "Zendesk API"\n5. Send Body : JSON\n6. Body :\n{\n  "ticket": {\n    "priority": "{{ $json.urgence === 'P1' ? 'urgent' : $json.urgence === 'P2' ? 'high' : 'normal' }}",\n    "tags": ["ia-triage", "{{ $json.categorie }}"],\n    "group_id": VOTRE_ID_EQUIPE,\n    "comment": {\n      "body": "🤖 Classification IA : {{ $json.categorie }} ({{ $json.urgence }})\\nSuggestion : {{ $json.suggestion_reponse }}",\n      "public": false\n    }\n  }\n}`,
        customization: `• Freshdesk : méthode PUT, URL https://votre-domaine.freshdesk.com/api/v2/tickets/{{ $json.ticketId }}, adaptez le body selon la doc Freshdesk\n• Crisp : utilisez le nœud natif "Crisp" de n8n pour mettre à jour la conversation\n• Sans outil de ticketing : remplacez ce nœud par un "Google Sheets" ou "Airtable" pour logger les classifications\n• Mappez vos group_id (équipes) : créez une table de correspondance dans un nœud Code entre vos catégories et vos IDs d'équipe`,
        errorHandling: `• Erreur 404 : l'ID du ticket est incorrect ou le ticket a été supprimé — vérifiez le format de l'ID dans le webhook source\n• Erreur 422 : le body JSON est malformé — vérifiez les guillemets et les expressions n8n {{ }}\n• Erreur 429 : rate limit Zendesk — ajoutez un "Wait" de 1s entre les appels si vous traitez en batch`,
      },
      {
        nodeLabel: "Slack — Notification équipe",
        nodeType: "Slack",
        nodeIcon: "💬",
        description: "Ce nœud envoie une notification Slack à l'équipe concernée. Pour les tickets P1, le message est envoyé immédiatement dans un canal d'urgence.",
        configuration: `1. Ajoutez un nœud "Slack"\n2. Resource : Message\n3. Operation : Send\n4. Channel : #support-triage (ou le canal de l'équipe cible)\n5. Text :\n🎫 Nouveau ticket classifié par l'IA\n📋 Catégorie : {{ $json.categorie }}\n🚨 Urgence : {{ $json.urgence }}\n👥 Équipe : {{ $json.equipe }}\n💡 Suggestion : {{ $json.suggestion_reponse }}\n🔗 Ticket : https://votre-zendesk.com/tickets/{{ $json.ticketId }}\n6. Authentication : créez un credential "Slack OAuth2" avec votre bot token`,
        customization: `• Pour les P1 : connectez la sortie 0 du Switch vers un Slack pointant sur #urgences avec @channel pour alerter tout le monde\n• Microsoft Teams : remplacez le nœud Slack par "Microsoft Teams" natif\n• Email : remplacez par un nœud "Send Email" (Gmail ou SMTP) si votre équipe préfère l'email\n• Discord : utilisez le nœud "Discord" natif avec un webhook de canal`,
        errorHandling: `• "channel_not_found" : le canal n'existe pas ou le bot n'y a pas accès — invitez le bot dans le canal avec /invite @votre-bot\n• "not_authed" : le token Slack est invalide — recréez le credential dans n8n\n• Message vide : vérifiez que les expressions {{ }} renvoient bien des valeurs dans l'onglet Output du nœud précédent`,
      },
    ],
    enterprise: {
